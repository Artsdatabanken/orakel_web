...
function UserFeedback({ inputStage, gotError }) {
  ...
  return (
    <React.Fragment>
      {inputStage && !gotError && (
        <div className="manual">
          ...
          <div className="manual-item">
            ...
            <div className="manual-item-text">{t("Short_manual.Zoom")}</div>
          </div>
          ...
          <div className="manual-item">
            ...
            <div className="manual-item-text">{t("Short_manual.Identify")}</div>
          </div>
        </div>
      )}
      ...
      {gotError === 503 && (
        <div className="hint">
          <div className="body emphasis">{t("Errors.503")}</div>
        </div>
      )}

      {gotError === 429 && (
        <div className="hint">
          <div className="body emphasis">{t("Errors.429")}</div>
        </div>
      )}
      ...
      {gotError && gotError !== 503 && gotError !== 429 && (
        <div className="hint">
          <div className="body emphasis">
            <Trans i18nKey="Errors.Generic">
              Error. Contact{" "}
              <a href="mailto:support@artsobservasjoner.no">e-mail link</a> if
              it persists.
            </Trans>

            {t("Errors.Code", { code: gotError })}
          </div>
        </div>
      )}
    </React.Fragment>
